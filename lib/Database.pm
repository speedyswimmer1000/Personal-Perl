...
our @EXPORT = qw(init about export_e_code del_all_table get_n_recent_rows_from_column mult_new_records info update_mult_rows_and_fields write_new read_all_data sum_col_w_args read_field read_field_one_val return_table_headers set_multiple_fields set_erase read_column search_val delete_row number_of_records); # A list of all functions - global vars also can go here.
...
our $SQL_LITE_MAX = 999;  # Maximum number of bind vars, defined in SQLite spec. 
$last_instance = -1;
...
sub get_n_recent_rows_from_column{
# Get the last n rows from a given column in the table, by the most recent.
# Will accept a higher number than the number of rows that exist, so be careful there. 
# Args: $dbhandle, $select_column, $select_value, $return_column, $num_entries_to_return
	if ((scalar @_ != 5) and (scalar @_ != 4)){
		return $e_code;
	}
	my $ref = shift(@_);
	if (set_pkg_instance($ref) == $e_code){
	...
	chomp ($select_column);
	if (is_valid_field($select_column) == $e_code){
	...
	my $return_column = shift @_;
	chomp ($return_column);
	if (is_valid_field($return_column) == $e_code){
		print "Valid field error.\n";
		return $e_code;
	}
	...
	my @bind_vars = ($select_value);
	my $records_query = qq/SELECT / . $return_column . " FROM " . $table . " WHERE " . $select_column . " = ? ORDER BY ROWID DESC";
	if (defined($num_entries)){
		$records_query .= " LIMIT ?"; 
		push(@bind_vars, $num_entries);
	}
	# Example: SELECT * FROM Day_table WHERE Symbol = "ABEV" ORDER BY ROWID DESC LIMIT 40
	#print $records_query;
	
	my $sql_handle = $dbhandle->prepare($records_query);  
	$sql_handle->execute(@bind_vars) or die $DBI::errstr;   #Execute query for the given symbol
	my $result = $sql_handle->fetchall_arrayref([]);  #Fetch all columns
	
	my @array = @$result;
	#print Dumper(@array);
	my @res_array;
	for (my $i = 0; $i < scalar @array; $i++){
		if (defined($array[$i][0])){
			push(@res_array, $array[$i][0]);  # Take out NULL values. 
		}
	}
...
#//Wrap here...
	...
	while (scalar @records_array > 0){
		...
		#$max_at_once = 80;
		our $num_records = 0;
		our @sub_records_array = ();
		if(scalar @records_array > $max_at_once){
			@sub_records_array = @records_array[0..$max_at_once - 1];
			splice @records_array, 0, $max_at_once;
		#	if (scalar @records_array >  $max_at_once){
		#		die "Too many records passed to this method.\n";
		#	}else{
			$num_records = scalar @sub_records_array; #(scalar @records_array); # I already put in one line in the query from the records array.
		#	}
		}
		else{
			@sub_records_array = @records_array;
			@records_array = ();  # So the while loop actually ends...
			$num_records = scalar @sub_records_array;
		}
	...
	#	8649367 AS Change_in_percent, 489346 as Avg_daily_volume, 3826 AS Fifty_day_avg, 3862936 AS One_yr_target, NULL AS Order_of_entry, 36238 AS Short_ratio
		...
		for (my $i = 0 ; $i < $num_records; $i++){
			push(@flat_array, @{$sub_records_array[$i]});
		}
...
sub update_mult_rows_and_fields{
# Inputs: $dbhandle, \@columns, $decision_column, \@symbols, \@@arrays_of_fields
	if (scalar @_ != 5 ){  # Use the ability to pass in ANDs/ORs, additional columns to select on, and data to select on.
	# There must be a number of arguments equal to 5 + 3*n.
		print "Invalid number of commands.\n";
		return $e_code;
	}
...
#	chomp $select_column;
	for (my $i = 0; $i < scalar @columns; $i++){
		if ((scrub($columns[$i]) == $e_code) or (is_valid_field($columns[$i]) == $e_code )){
			print "Selection column not found in this table.\n";
			return $e_code;
		}
	}
	our $decision_column = shift(@_);
	my @symbols = @{shift(@_)};
	my @read_fields = @{shift(@_)}; # Still an array of arrays. 
	
	if (scalar @symbols != scalar @{$read_fields[0]}){
		print "Symbols and number of fields  to insert are not the same.\n";
		return $e_code;
	}
	for (my $i = 0; $i < scalar @read_fields; $i++){
		if (scalar @{$read_fields[0]} != scalar @{$read_fields[$i]}){
			print "Not the same number of entries everywhere. $i\n";
			print scalar @{$read_fields[0]} . " " . @{$read_fields[$i]};
			return $e_code;
		}
	}
	if (scalar @columns != scalar @read_fields){
		print "Not enough data sets for every column.\n";
		return $e_code;
	}
	
	
	my $bind_vars_per_record = 2 * (scalar @columns) + 1; # For WHEN Symbol = ? THEN ?, every column and WHERE Symbol in (?) at the end.
	print "\n" . scalar @columns . "\n";
	our $records_at_once = floor($SQL_LITE_MAX / $bind_vars_per_record);
print "$records_at_once records at once $bind_vars_per_record \n";
	
	my $num_symbols = scalar @symbols;
	print "\n$num_symbols";
	#my $val = <>;
##Haha, my methods here get more compact and convoluted. JAPH, I guess. I'm attempting to set up something of this form:
=pod
...
=cut
#In order to do this, I start off with my UPDATE .. SET. 
# Records_at_once has been calculated to fit the max number of bind variables (999 for SQLite) that I can put in one query.
#This is two bind vars for every field per record, plus one at the end for the WHERE Symbol in (...). 
	for (our $i = 0; $i < $num_symbols; $i += $records_at_once){
	#Here, I'm iterating through the whole list of symbols that I've passed in - or in other words, the number of @symbols, 
	# doing $records_at_once records at a time. 
	#print "\n$i";
#	my $val = <>;
		our @bind_vars = ();  #Reset my bind_vars
		our $sql_statement = qq/UPDATE / . $table . " SET ";
		our $sql_case_statement = "";
		our @local_symbols = ();
		if (scalar @symbols > $records_at_once){  #Trim off the first $records_at_once elements and put in array @local_symbols.
			@local_symbols = @symbols[0..$records_at_once - 1];
			splice @symbols, 0, $records_at_once;  # Remove the first max_at_once elements
			
		}else{
			@local_symbols = @symbols;  # If I've got less than $records_at_once.
		}
		for (my $j = 0; $j < scalar @columns; $j++){  # Each SET *col* = CASE .... END statement is formed here. 
			my $cur_column = $columns[$j];  # Get the current column I'm working on.
			$sql_case_statement .= $cur_column . "=CASE ";
			for (my $k = 0; $k < scalar @local_symbols; $k++){
				$sql_case_statement .= "WHEN " . $decision_column . " = ? THEN ";# . $local_symbols[$k] . " THEN ";
				# So decision column has to equal the symbol, which is held in @local_symbols. 
				# I'm iterating through the whole length of @local_symbols.
				push(@bind_vars, $local_symbols[$k]);
				# Push into bind_vars, because it matches the question mark. 
				
				#print "$sql_case_statement\n";
				#print Dumper(@read_fields);
				#print Dumper(${$read_fields[$j]}[$k]);
				
				$sql_case_statement .= "? ";#${$read_fields[$j]}[$k] . " ";  #Hacky...
				push(@bind_vars, ${$read_fields[$j]}[($k + $i)]);  # So I don't repeat the same info over and over.
								# Since $i already increases in multiples of $records_at_once, I don't have to multiply
								# again. The parentheses around k+i are important somehow too.
				# Push the value of the field we're replacing in the bind_vars so that that record's field is replaced. 
			}			
			$sql_case_statement .= "END, \n"; # Ready for next column to replace. 
		} # End of doing the WHEN... columns
		$sql_case_statement =~ s/,\s\n$/ /g;  # Take out last comma.
		$sql_case_statement .= "WHERE " . $decision_column . " IN (";   # . join(',',@local_symbols) . ")";
		my $questions = "";
		for (my $j = 0; $j < scalar @local_symbols; $j++){
		### Bind vars at the end for which columns I want to touch. This is important because if I don't have that part, it nukes the rest of
		# the entries in the table in that column.
			$questions .= "?, ";
			push(@bind_vars, $local_symbols[$j]);
		}
		$questions =~ s/,\s$/ /g; # Again, take out last comma.
		$sql_case_statement .= $questions . ")";
		$sql_statement .= $sql_case_statement;
		#print "$sql_statement\n";
		#print join(',', @bind_vars);
		
		my $sub_handle = $dbhandle->prepare($sql_statement);
		$sub_handle->execute(@bind_vars);
	#	my @bind_vars = 
	}
}
